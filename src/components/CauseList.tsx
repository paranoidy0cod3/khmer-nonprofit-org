...
const CauseList = () => {
  const causes = [
    ...
    return (
        ...
        >
          <LuHelpingHand size={40} className='inline-block mr-2' /><h2 className='font-semibold text-2xl inline-block'>Our Causes</h2> <Link className='text-sky-400 inline-block' to="/causes">View All Causes</Link>
            {/* Navigation Buttons */}
            ...
            <div className="cause-list-wrapper flex transition-transform duration-500 ease-in-out" style={{ transform: `translateX(-${currentIndex * 100}%)` }}>
                {causes.map((cause, index) => (
                    <div key={index} className="cause-card-wrapper flex-shrink-0 w-full md:w-1/4 p-4">
                        <CauseCard {...cause} />
                    </div>
                ))}
            </div>
